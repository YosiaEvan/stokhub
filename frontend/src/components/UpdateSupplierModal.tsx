...
export default function UpdateSupplierModal({ setIsOpen, supplier, setRefreshKey }: UpdateSupplierModalProps) {
    ...
    const handleUpdateSupplierSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const payload: any = {
                nama,
            ...
            const response = await api.put(`/supplier/${supplier.id}`, payload);
        ...
        <div className="absolute inset-0 z-50 bg-black/40 w-full flex items-center justify-center p-6">
            <div className="bg-white rounded-xl w-full max-h-full max-w-2xl flex flex-col overflow-hidden">
                <div className="flex items-center justify-between border-b-2 border-[#f4f4f5] p-5 shrink-0">
                    <h3 className="text-xl font-semibold">Update Supplier</h3>
